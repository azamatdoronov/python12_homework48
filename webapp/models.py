...
CATEGORY_CHOICES = [('DEFAULT_CATEGORY', 'разное'), ('bakery', 'выпечка'), ('sweets', 'сладости'),
...
class Product(models.Model):
    ...
    p_name = models.CharField(max_length=100, verbose_name='Название')
    ...
    def str(self):
        return f"{self.p_name} - {self.balance}"
...
class Cart(models.Model):
    ...
    def __str__(self):
        return f'{self.product.p_name} - {self.qty}'
...
class OrderProduct(models.Model):
    ...
    def __str__(self):
        return f'{self.product.p_name} - {self.order.name}'
